...
final class Http2Ext(private val config: Config)(implicit val system: ActorSystem)
  ...
  def bindAndHandleAsync(
    ...
    val http1 = Flow[HttpRequest].mapAsync(parallelism)(handleUpgradeRequests(handler, settings, parallelism, log)).joinMat(http.serverLayer(settings, None, log))(Keep.left)
